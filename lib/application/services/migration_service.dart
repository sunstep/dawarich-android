...
final class MigrationService {
  ...
  MigrationService(this._driftDatabase, final Store _obxDb) :
    _steps = [
      DriftToObxMigration(_driftDatabase, _obxDb)
    ];
  ...
  Future<bool> needsMigration() async {
    ...
    for (final step in _steps) {
      ...
      try {
        ...
        if (kDebugMode) {
          debugPrint('[MigrationService] failed to check step ${step
              .fromVersion}→${step.toVersion}: $e'
          );
        }
      }
  ...
  Future<void> runIfNeeded() async {
    ...
    if (pendingVersions.isEmpty) {
      if (kDebugMode) {
        debugPrint('[MigrationService] No migrations pending; database is up-to-date.');
      }
      return;
    }
    ...
    if (kDebugMode) {
      debugPrint('[MigrationService] Starting migrations at version $firstVersion');
    }
    ...
    while (step != null) {
      ...
      if (kDebugMode) {
        debugPrint('[MigrationService] Successfully applied migration ${step.fromVersion} → ${step.toVersion}');
      }
    ...
    if (kDebugMode) {
      debugPrint('[MigrationService] All done, final version = $currentVersion');
    }
