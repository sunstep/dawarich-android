...
final class MigratePoints {
  ...
  Future<Result<(), String>> startMigration() async {
    ...
    if (driftRowCount == 0) {
      return Err(
          "[DriftToObjectbox] Point migration was not necessary due to empty Drift table");
    }
    ...
    if (pointBox.count() == driftRowCount) {
      return Err(
          "[DriftToObjectbox] Point migration was not necessary due to the obx database having the same data as ");
    }

    final List<PointEntity> migratedTable = allDriftRows.map((row) {
      final migratedPoints =
          PointEntity(id: row.id, type: row.type, isUploaded: row.isUploaded);
