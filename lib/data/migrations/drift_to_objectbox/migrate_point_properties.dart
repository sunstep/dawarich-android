...
final class MigratePointProperties {
  ...
  Future<Result<(), String>> startMigration() async {
    final allDriftRows =
        await _driftDb.select(_driftDb.pointPropertiesTable).get();
    final int driftRowCount = allDriftRows.length;

    if (driftRowCount == 0) {
      return Err(
          "[DriftToObjectbox] Point properties migration was not necessary due to empty Drift table");
    }

    final Box<PointPropertiesEntity> pointPropertiesBox =
        _obxDb.box<PointPropertiesEntity>();

    if (pointPropertiesBox.count() == driftRowCount) {
      return Err(
          "[DriftToObjectbox] Point properties migration was not necessary due to the obx database having the same data as ");
    }
