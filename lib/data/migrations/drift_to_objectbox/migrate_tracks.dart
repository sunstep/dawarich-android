...
import 'package:dawarich/data/objectbox/entities/track/track_entity.dart';
...
final class MigrateTracks {
  ...
  Future<Result<(), String>> startMigration() async {
    ...
    if (driftRowCount == 0) {
      return Err("[DriftToObjectbox] Track migration was not necessary due to empty Drift table");
    }

    final Box<TrackEntity> trackBox = _obxDb.box<TrackEntity>();

    if (trackBox.count() == driftRowCount) {
      return Err("[DriftToObjectbox] Track migration was not necessary due to the obx database having the same data as ");
    }
