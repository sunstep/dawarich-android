...
final class MigrateUsers {
  ...
  Future<Result<(), String>> startMigration() async {
    ...
    if (driftRowCount == 0) {
      return Err(
          "[DriftToObjectbox] user migration was not necessary due to empty Drift table");
    }
    ...
    try {
      ...
      if (userBox.count() == driftRowCount) {
        return Err(
            "[DriftToObjectbox] User migration was not necessary due to the obx database having the same data as the Drift database");
      }
