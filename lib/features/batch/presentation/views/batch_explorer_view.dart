...
final class BatchExplorerPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final vmAsync = ref.watch(batchExplorerViewModelProvider);
    return vmAsync.when(
      loading: () => Container(
      ...
      data: (vm) => provider.ChangeNotifierProvider<BatchExplorerViewModel>.value(
        value: vm,
        child: const _BatchExplorerScaffold(),
      ),
    );
  }
}

final class _BatchExplorerScaffold extends StatefulWidget {
  const _BatchExplorerScaffold();

  @override
  State<_BatchExplorerScaffold> createState() => _BatchExplorerScaffoldState();
}

class _BatchExplorerScaffoldState extends State<_BatchExplorerScaffold> {
  StreamSubscription<String>? _uploadSub;
  bool _isShowingUploadError = false;

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();

    // Attach once when the Provider-based VM becomes available.
    _uploadSub ??= context.read<BatchExplorerViewModel>().uploadResultStream.listen((msg) {
      if (!mounted) return;
      if (_isShowingUploadError) return;

      _isShowingUploadError = true;
      showDialog(
        context: context,
...
class _BatchContent extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return provider.Consumer<BatchExplorerViewModel>(
                            ...
                            itemBuilder: (_, i) {
                              ...
                              return Padding(
                                ...
                                child: _PointCard(
                                  ...
                                  onDelete: () => _Dialogs.confirmDeletePoint(
                                      context, vm, pt),
                                ),
                              );
                            },
