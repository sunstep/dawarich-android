...
AsyncNotifierProvider<CountriesViewmodel, VisitedCountriesUiModel?>(
  CountriesViewmodel.new,
);

final class CountriesViewmodel extends AsyncNotifier<VisitedCountriesUiModel?> {
  CountriesQuery? _lastQuery;

  @override
  Future<VisitedCountriesUiModel?> build() async {
    final useCase = await ref.watch(getVisitedCountriesUseCaseProvider.future);
    final mapper = ref.watch(countriesUiMapperProvider);
  ...
  Future<VisitedCountriesUiModel?> _fetchCountries(
    ...
    if (result case Ok(value: final countries)) {
      return mapper.convert(countries);
    }
