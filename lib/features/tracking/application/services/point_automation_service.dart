...
final class PointAutomationService with ChangeNotifier {
  ...
  Future<void> startStreamSubscription() async {
    if (_stream == null || _stream!.isPaused) {
      ...
      debugPrint(
          "[DEBUG: Position stream] A position stream is already being listened to, not starting another one");
    }
  }
  ...
  Future<void> _streamHandler(Result<Position, String> result) async {
    if (result case Ok(value: Position position)) {
      final Result<LocalPoint, String> storeResult =
          await _localPointService.createAndStorePoint(position);
